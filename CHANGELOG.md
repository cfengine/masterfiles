3.18.5:
	...
	- Added guards against using regline() in cases where a file may not exist
	  (ENT-9933)
	- Added policy to enforce proper permissions on Mission Portal ldap directory
	  (ENT-9693)
	- Added ssl_request_log to list of hub log files (ENT-10192)
	- Fixed body perms system_owned to account for Windows (ENT-9778)
	- Improved federated reporting dump concurrency with database (ENT-10214)
...
3.18.3:
	...
	- Added date binary to known paths for linux (CFE-4069)
	- Added fallback to top-level feeder dump directory (ENT-8936)
	- Fixed self upgrade for Suse 12, 15 and opensuse leap 15
	  (ENT-9209)
	- Fixed self upgrade for debian 11 (ENT-9210)
